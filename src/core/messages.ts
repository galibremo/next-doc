export const authMessages = {
  success: {
    ...
    accountVerification:
      "Account verification successful. Your account is active.",
    passwordChange:
      "Password change successful. Your password has been updated.",
  ...
  error: {
    ...
    passwordReset: {
      invalidToken:
        "Password reset failed. The reset token is invalid or expired.",
      userNotFound:
        "Password reset failed. No user found with this email address.",
    ...
    emailVerification: {
      ...
      alreadyVerified:
        "Email verification failed. The email is already verified.",
      serverError: "Email verification failed. Server error occurred.",
    },
    passwordChange: {
      incorrectPassword:
        "Password change failed. The current password is incorrect.",
...
export const zodMessages = {
  error: {
    ...
    limit: {
      ...
      arrayMax: (field: string, limit: number) =>
        `${field} must not exceed ${limit} items.`,
      numberMin: (field: string, limit: number) =>
        `${field} must be at least ${limit}.`,
      numberMax: (field: string, limit: number) =>
    ...
    invalid: {
      ...
      invalidUnion: (field: string) =>
        `${field} must be one of the specified types.`,
      ...
      invalidFileType: (field: string, type: string) =>
        `${field} must be of type ${type}.`,
      invalidUpperCase: (field: string) =>
        `${field} must be at least one upper case.`,
      invalidLowerCase: (field: string) =>
        `${field} must be at least one lower case.`,
      invalidNumericCase: (field: string) =>
        `${field} must be at least one number.`,
      invalidPhoneNumber: (field: string) =>

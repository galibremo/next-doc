# Next.js Stack Developer Documentation
...
# 10. Domain with Multiple Resources Pattern
...
# 14. URL State Standard with nuqs
...
# 31. Adding a New List/Table Feature
...
# 34. Files to Avoid Changing Casually
...
# 35. Do Not Add Without Approval
...
- New global providers for feature-only concerns.
- Barrel files that obscure ownership.
...
Project-specific scripts may vary. Check `package.json` before adding new scripts.
...
- Feature code lives under the correct `src/features` folder.
- Shared UI is actually shared.
- Domain-specific UI stays inside the feature.
- API calls are in feature `api` files.
- Query keys are centralized.
- Query hooks use TanStack Query.
- Mutations invalidate or update related queries.
- URL state uses `nuqs`.
- Forms use React Hook Form and Zod.
- Shared validators are reused where appropriate.
- Backend errors are handled through feature error utilities.
- Auth and role checks do not bypass backend authorization.
- Route constants are used instead of scattered path strings.
- Lockfile changed only because dependencies changed.
- Documentation or memory files are updated when structure or conventions change.

---

# 38. Pattern Copy Guide

When building the next feature, start by finding the closest existing pattern in the same project.

| New work | Copy the pattern from |
| --- | --- |
| Simple page | Existing thin route plus feature page component |
| List page | Existing list page with context, query hook, table, toolbar |
| Detail page | Existing dynamic route and detail query pattern |
| Form | Existing feature form using React Hook Form and Zod |
| API calls | Existing feature `*.api.ts` file |
| Query keys | Existing feature `*.keys.ts` file |
| Queries | Existing feature `*.queries.ts` file |
| Mutations | Existing feature `*.mutations.ts` file |
| Error mapping | Existing feature or domain error utility |
| Shared UI | Existing `src/components/common` or domain shared component |

If the new work does not fit any existing pattern, document the proposed structure in `docs/plan` before implementing it.

---

# 39. Developer Workflow for New Work

## Before Coding

1. Identify the domain or standalone feature.
2. Find the closest existing pattern.
3. Confirm route constants and backend endpoint paths.
4. Decide where state belongs:
   - API data: TanStack Query.
   - URL filters: nuqs.
   - Form values: React Hook Form.
   - Feature orchestration: feature context.
   - Small UI toggles: local component state.
5. Define schemas and types before wiring UI.

## While Coding

1. Keep the route file thin.
2. Add or update feature API calls.
3. Add or update query keys.
4. Add query/mutation hooks.
5. Build feature components.
6. Wire forms to schemas and mutations.
7. Wire tables to context and shared table components.
8. Add error handling through feature utilities.
9. Avoid changing global files unless required.

## After Coding

1. Run lint.
2. Run build.
3. Consider TypeScript no-emit check.
4. Run tests when available.
5. Review the PR checklist.
6. Update docs if structure, workflows, routes, or conventions changed.
...
```tsx
export default function Page() {
	// filters, tables, mutations, modals, forms, API calls...
}
```
...
```tsx
const { data, isLoading } = useResourcesQuery(filters);
```

## Feature Logic in Shared UI
...
```tsx
// src/features/resources/components/resource-action-button.tsx
...
## Duplicate Table Frameworks

Bad:

```text
src/features/resources/components/custom-table-system/*
```
...
src/components/common/table/data-table.tsx
```

---

# 41. Example Project Mappings

The same structure works across different project types.
...
These are examples, not required names. The rule is to organize by domain ownership, not by random technical categories.

---

# 42. Final Team Rule

Every new implementation should answer these questions before merging:

1. Does the route stay thin?
2. Does the feature own its domain behavior?
3. Is reusable UI actually reusable?
4. Is API state handled through TanStack Query?
5. Is URL state handled through nuqs?
6. Are forms using React Hook Form and Zod?
7. Are backend business rules still authoritative?
8. Did we copy the closest existing pattern instead of inventing a new one?
9. Would another developer know where to add the next related change?
